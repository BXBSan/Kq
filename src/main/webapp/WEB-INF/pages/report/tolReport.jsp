...
<head>
  ...
  <script type="text/javascript">
    ...
    function doexport(){
      ...
      var empbox = document.getElementsByName("emp");
      var depts = "";
      var emps = "";
      for(var i = 0;i<deptbox.length;i++) {
        if(deptbox[i].checked) {
          depts += ("&dept=" + deptbox[i].value)
        }
      }
      for(var i = 0;i<empbox.length;i++) {
        if(empbox[i].checked) {
          emps += ("&emp=" + empbox[i].value)
        }
      }
      location.href="${pageContext.request.contextPath}/report/export?nowDate=${nowDate}"+depts+emps;
    }
    $(function(){
      obj = eval('( ${deptTree})');
    ...
    /*function traverse(node, i) {
      var children = node.sdept;
      dept += node.deptName;
      if (children != null) {
        alert("parent:" + node.deptName + ",child:" + children[i].deptName);
  ...
  <div class="admin-content">
    <div class="admin-content-body">
      ...
      <ul class="am-avg-sm-1 am-avg-md-4 am-margin am-padding am-text-center admin-content-list ">
        ...
        <li><a href="${pageContext.request.contextPath}/report/getCheckReport" class="am-text-success"><span class="am-icon-btn am-icon-file-text"></span><br/>考勤报表</a><button  type="button" class="am-btn am-btn-success am-btn-xs" style="margin-right:-90px;margin-left:30px;" onclick="doexport()" ><span class="am-icon-save"></span>导出</button></li>
    ...
    <form action="${pageContext.request.contextPath}/report/getAll">
      ...
      <div class="am-u-sm-12 am-u-md-3">
        <div class="am-input-group am-input-group-sm">
          <input name="date" value="${nowDate}" id="hiredate" type="text" class="am-form-field" placeholder="查询时间" readonly="true" />
      ...
      <div class="am-g">
        <div class="am-u-sm-12">
          <table class="am-table am-table-bd am-table-striped admin-content-table">
            ...
            <tbody>
            <c:forEach items="${tolReports}" var="aa" varStatus="status">
              <tr>
                <td><input type="checkbox" name="emp" value="${aa.uuid}"/></td>
                <td>${aa.empNo}</td>
                <td><a style="color: green;" href="${pageContext.request.contextPath}/report/getApplyDetail?empId=${aa.uuid}&nowDate=${nowDate}">${aa.empName}</a></td>
                <td>${aa.clockNum}</td>
                <td class="am-hide-sm-only">${aa.leaveNum}</td>
                <td>${aa.overTime}</td>
                <td>${aa.offNum}</td>
              </tr>
            </c:forEach>
...
<script type="text/javascript">
  function checkIn(e){
    var check = document.getElementsByName("emp");
